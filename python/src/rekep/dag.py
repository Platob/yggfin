...
Tasks are referenced by URI (`job:/pipeline/files_to_logs`), never restated:
...
class Dag(Record):
    ...
    uri: str
    """This dag's identity as a path: `dag:/namespace/name`.

    The same spelling a job and a dataset use, scoped to its own service --
    so `dag:/pipeline/trading_logs` and `job:/pipeline/trading_logs` are two
    resources, not one name used twice."""

    tasks: list[str] = dataclasses.field(default_factory=list)
    """The tasks in this dag, as job URIs (`job:/pipeline/files_to_logs`).
    ...
    def resource_uri(self) -> ResourceUri:
        """This dag's identity: `dag:/namespace/name`."""
