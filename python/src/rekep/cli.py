"""The rekep command line, organised by service.

Every command hangs off a service (`rekep service ddl dump ...`), so a new
capability is a new service class here rather than a new flag on an old one.
...
class DdlService:
    """`rekep service ddl`: DDL emitted from record declarations."""

    name = "ddl"

    def register(self, services: Any) -> None:
        parser = services.add_parser(self.name, help="DDL from record declarations")
...
class ProductService:
    """`rekep service product`: record declarations as reviewable files."""

    name = "product"

    def register(self, services: Any) -> None:
        parser = services.add_parser(self.name, help="record declarations as files")
        commands = parser.add_subparsers(dest="command", required=True)
...
class DocsService:
    """`rekep service docs`: documentation pages generated from the code."""

    name = "docs"

    def register(self, services: Any) -> None:
        parser = services.add_parser(self.name, help="generated documentation pages")
        commands = parser.add_subparsers(dest="command", required=True)
...
class DorisService(StackService):
    """`rekep service doris`: the Doris stack, resource by resource."""
...
class RecordsService:
    """`rekep service records`: one record class, converged into the stacks."""

    name = "records"

    def register(self, services: Any) -> None:
        parser = services.add_parser(self.name, help="deploy a record class to the stacks")
        commands = parser.add_subparsers(dest="command", required=True)
...
class AirflowService:
    """`rekep service airflow`: DAG modules as a deployable resource."""

    name = "airflow"

    def register(self, services: Any) -> None:
        parser = services.add_parser(self.name, help="deployable Airflow DAG modules")
        commands = parser.add_subparsers(dest="command", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    services = top.add_parser("service", help="service commands").add_subparsers(
