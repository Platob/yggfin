"""What a field declaration says: its type hints and its documentation.

Both projections read the same two things off a record -- what a field is
declared as, and what it is documented as -- and so does the decoder that
rebuilds one from a file. That shared reading lives here, so neither projection
has to depend on the other to reach it.
"""
...
def unwrap_annotated(annotation: Any) -> tuple[tuple[Any, ...], Any]:
    ...
    What those extras *mean* is each projection's business; the unwrapping both
    of them need is all that happens here.
    """
...
def docstring_attributes(cls: type) -> dict[str, str]:
    """Field descriptions for `cls`, gathered from every source, weakest first.

    A description written under its own field wins over one collected into an
...
def _attribute_docstrings(cls: type) -> dict[str, str]:
    """Descriptions written as a bare string literal directly under a field::
    ...
    described: dict[str, str] = {}
    for field, following in itertools.pairwise(parsed.body):
        ...
        if isinstance(following, ast.Expr) and isinstance(following.value, ast.Constant):
            if isinstance(following.value.value, str):
                described[field.target.id] = _fold(following.value.value)
    return described
    ...
    while index < len(lines):
        ...
        while index < len(lines):
            ...
            if entry:
                ...
                current = None
            index += 1
    return {name: _fold(text) for name, text in described.items() if text}
