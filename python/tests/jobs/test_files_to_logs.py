...
def test_arrow_transform_is_the_identity() -> None:
    ...
    (out,) = list(FilesToLogs(name="f2l").arrow_transform(iter([batch])))
    assert out is batch


def test_run_parses_the_sample_file() -> None:
    job = FilesToLogs(name="f2l", source=SAMPLE.as_uri())
    assert job.run() == 24
...
def test_is_a_record_and_round_trips() -> None:
    job = FilesToLogs(name="f2l", schedule="@daily", source=SAMPLE.as_uri())
