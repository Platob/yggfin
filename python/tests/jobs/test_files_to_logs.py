...
def test_arrow_transform_is_the_identity() -> None:
    ...
    (out,) = list(FilesToLogs(uri="rekep:///jobs/f2l").arrow_transform(iter([batch])))
    assert out is batch


def test_run_parses_the_sample_file() -> None:
    job = FilesToLogs(uri="rekep:///jobs/f2l", source=SAMPLE.as_uri())
    assert job.run() == 24
...
def test_a_run_event_reports_log_as_the_output() -> None:
    ...
    job = FilesToLogs(uri="rekep:///jobs/pipeline/f2l", source=SAMPLE.as_uri())
...
def test_is_a_record_and_round_trips() -> None:
    job = FilesToLogs(uri="rekep:///jobs/f2l", schedule="@daily", source=SAMPLE.as_uri())
