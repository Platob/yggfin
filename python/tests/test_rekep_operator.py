"""The operator Airflow runs this package's bundled tasks with.
...
def operator(**held: Any) -> Any:
    ...
    held.setdefault("task_name", "parse_messages")
    held.setdefault("repository", str(ROOT))
    return RekepOperator(**held)
...
def test_the_ingestion_dag_fans_out_from_one_committed_book_stage() -> None:
    ...
    # Params mapping over every task's defaults hands neither the other's source.
...
def test_every_node_runs_the_bundled_task_it_is_named_for() -> None:
    """A node publishes, as its outlets, exactly the tables its task writes."""
    from rekep.tasks import Task

    for dag in (PIPELINE.ingestion, PRODUCTS.products):
        for node in dag.tasks:
            assert node.task_name == node.task_id
            assert tuple(asset.name for asset in node.outlets) == Task(node.task_name).targets
    assert PRODUCTS.PUBLISHED == Task("build_dbt").targets


def test_the_dags_read_the_defaults_the_package_ships() -> None:
    """Each DAG reads a task's JSON off the checkout, the same document
    ...
    parsed = subprocess.run(  # noqa: S603
        ...
        env={**os.environ, "PYTHONPATH": str(DAGS)},
        capture_output=True,
        text=True,
        check=True,
    )
    assert parsed.stdout.strip() == "[]", parsed.stdout + parsed.stderr


# -- the command it builds ---------------------------------------------------


def test_the_task_command_runs_the_bundled_task_through_the_rekep_cli() -> None:
...
def test_the_shipped_defaults_reach_the_task(kept: Held) -> None:
    operator().execute(context())
...
def test_the_defaults_are_the_checkout_the_child_runs(kept: Held, tmp_path: Path) -> None:
    """Not the worker's own `rekep`: the checkout `repository` names is what the
    child runs and what the DAG reads its Params from, so it is what the
    operator merges on too -- a name only that checkout declares is kept."""
    shipped = tmp_path / "python" / "src" / "rekep" / "tasks"
    shipped.mkdir(parents=True)
    (tmp_path / "python" / "pyproject.toml").write_text("", encoding="utf-8")
    defaults = {**Task("parse_messages").parameters, "filesystem": "file:elsewhere", "added": 1}
    (shipped / "parse_messages.json").write_text(json.dumps(defaults), encoding="utf-8")

    operator(repository=str(tmp_path)).execute(context(params={"added": 2}))

    parameters = written()
    assert parameters["filesystem"] == "file:elsewhere"
...
def test_a_task_name_that_is_not_a_module_name_is_refused(name: str) -> None:
    ...
    with pytest.raises(AirflowException, match="no task is named"):
        operator(task_name=name).execute(context())
    assert not Ran.calls


def test_a_param_the_task_does_not_declare_is_not_injected(kept: Held) -> None:
    """Only parameters message ingestion declares reach its run."""
...
def test_the_interval_fills_a_declared_start_and_end(kept: Held) -> None:
    ...
    built = operator(task_id="build_dbt", task_name="build_dbt")
...
def test_an_explicit_parameter_is_overridden_by_the_param_of_the_same_name(
    ...
    """Sources merge once, in one order: defaults, operator, Params, interval."""
...
def test_fanout_pins_the_actual_parent_window_snapshot_and_table(
    ...
    parent = operator(task_id="parse_books", task_name="parse_books")
    ...
    child = operator(
        task_id="parse_orders", task_name="parse_orders", upstream_task_id="parse_books"
    )
    child.execute(
        context(
            params={
...
def test_an_explicit_parameter_the_task_does_not_declare_is_refused_before_launch() -> None:
    from airflow.sdk.exceptions import AirflowException

    with pytest.raises(AirflowException, match="parse_messages takes no bokos; it takes"):
        operator(parameters={"bokos": True}).execute(context())
    assert not Ran.calls


def test_a_handoff_to_a_task_without_a_window_is_refused_before_launch() -> None:
    """Only a task taking `start`, `end` and `snapshot_id` can follow a snapshot."""
    from airflow.sdk.exceptions import AirflowException

    child = operator(task_id="build_dbt", task_name="build_dbt", upstream_task_id="parse_books")
    with pytest.raises(AirflowException, match="build_dbt takes no end, snapshot_id, start"):
...
@pytest.mark.parametrize(
    "published",
    [{"task": "parse_messages"}, {**RESULT, "read": -1}, {**RESULT, "window": {}}, [1, 2]],
...
def test_a_task_the_package_does_not_bundle_is_refused_before_launch(kept: Held) -> None:
    from airflow.sdk.exceptions import AirflowException

    with pytest.raises(AirflowException, match="no task is named 'parse_nothing'"):
        operator(task_name="parse_nothing").execute(context())
    assert not Ran.calls
    assert not kept.paths, "and no attempt directory was made for it"


def test_the_task_name_is_rendered_before_it_is_checked() -> None:
    """`task_name` is a template field, so a run may name the task it runs."""
    built = operator(task_name="{{ params.stage }}")
    built.render_template_fields({"params": {"stage": "parse_fix_raw"}})

    built.execute(context())

    (run,) = commands()
    assert run[run.index("tasks") + 1 : run.index("--parameters-file")] == ["parse_fix_raw", "run"]
...
def _pass(catalog: dict[str, Any], capture: Path) -> dict[str, dict[str, Any]]:
    ...
    The task, outlets and handoff come off the shipped DAG rather than being
    spelled again here, so a node that stops declaring what it runs or the
    Asset it writes fails this test.
    """
    landed = {}
...
def test_the_scheduled_graph_publishes_the_market_capture_and_replays_it(
    ...
    """The whole scheduled route: uv, the rekep CLI, seven tasks, seven tables.
...
def test_a_scheduled_result_is_the_shape_a_route_reads(
    ...
    for name, held in landed.items():
        ...
        # `window` is a mapping in every result, never null: both the CLI and
        # the operator validate it, so the contract is the same both ways.
...
@pytest.mark.integration
def test_terminating_the_task_stops_the_child_process_group(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """`on_kill` signals the process group, so nothing is left holding a table.

    The child is the operator's own `uv run` line with a sleeper in place of
    the task, which outlives the test unless the whole group is signalled.
    """
    monkeypatch.undo()
    command = RekepOperator._argv

    def sleeping(self: Any, *held: Any) -> list[str]:
        launched = command(self, *held)
        return [
            *launched[: launched.index("--") + 1],
            "python",
            "-c",
            "import time; time.sleep(600)",
        ]

    monkeypatch.setattr(RekepOperator, "_argv", sleeping)
    built = operator()
    ...
    assert not subprocess.run(
        ["pgrep", "-g", str(group)], capture_output=True, text=True, check=False
    ).stdout.split(), "uv and its process group are gone"
...
def test_a_real_dag_run_publishes_its_seven_tables_from_its_conf(
    ...
    # Every node ran its task through the rekep CLI, and the run's conf
    # reached them: the catalog they wrote is this test's, not the relative
    # default the package ships.
    for task_id in PUBLISHED:
        assert repr(["rekep", "tasks", task_id, "run"])[1:-1] in printed, printed
    store = IcebergCatalog.from_dict(catalog)
    try:
        stored = {
            dataset.identifier: dataset.read_arrow_table().num_rows
