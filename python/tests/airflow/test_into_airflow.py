...
def test_a_dag_becomes_one_airflow_dag_with_one_task_per_job(sdk: Any) -> None:
    declare("job:/pipeline/extract")
    declare("job:/pipeline/load")
    built = Dag(
        uri="dag:/pipeline/demo",
        tasks=["job:/pipeline/extract", "job:/pipeline/load"],
...
def test_each_task_runs_its_own_job(sdk: Any) -> None:
    ...
    (task,) = Dag(uri="dag:/pipeline/demo", tasks=[job.uri]).into_airflow().tasks
...
def test_airflow_config_passes_straight_through(sdk: Any) -> None:
    ...
    declare("job:/pipeline/extract", airflow={"task": {"retries": 3, "pool": "etl"}})
    built = Dag(
        uri="dag:/pipeline/demo",
        tasks=["job:/pipeline/extract"],
...
def test_a_task_overrides_the_dags_own_task_defaults(sdk: Any) -> None:
    declare("job:/pipeline/extract", airflow={"task": {"retries": 3}})
    declare("job:/pipeline/load")
    built = Dag(
        uri="dag:/pipeline/demo",
        tasks=["job:/pipeline/extract", "job:/pipeline/load"],
...
def test_lineage_is_derived_not_declared(sdk: Any) -> None:
    ...
    built = Dag(
        uri="dag:/pipeline/demo", tasks=["job:/pipeline/parse"], tags={"mine": "yes"}
    ).into_airflow()
...
def test_a_dag_declaring_no_lineage_gets_no_assets(sdk: Any) -> None:
    declare("job:/pipeline/extract")
    (task,) = Dag(uri="dag:/pipeline/demo", tasks=["job:/pipeline/extract"]).into_airflow().tasks
...
def test_a_single_job_dag_needs_no_side_file(sdk: Any) -> None:
    ...
    job = declare("job:/pipeline/extract", schedule="@hourly", tags={"stage": "ingestion"})
