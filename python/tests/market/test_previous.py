...
def _order(_instrument: Instrument = EQUITY, **given: object) -> Order:
    ...
    return Order(**{"instrument_xhash": _instrument.xhash, **given}).attach_instrument(_instrument)
...
def _execution(_instrument: Instrument = EQUITY, **given: object) -> Execution:
    ...
    values = {"instrument_xhash": _instrument.xhash, **given}
...
def test_a_message_with_no_clock_lands_where_the_version_before_it_was() -> None:
    ...
    assert timeless.unix_partition == first.unix_partition, "and the partition follows the time"
...
def test_a_correction_is_another_version_of_one_execution_not_a_second_one() -> None:
    ...
    assert fixed.exec_id == "EX-2" and fixed.code == "EX-1"
...
def test_the_same_readable_lifecycle_code_is_scoped_before_it_is_hashed() -> None:
    here = _order(instrument_xhash=1, mic=XNAS, code="ORD-1").with_previous(None)
    there = _order(instrument_xhash=2, mic=XNAS, code="ORD-1").with_previous(None)
    elsewhere = _execution(instrument_xhash=1, mic=XNAS, code="ORD-1").with_previous(None)
