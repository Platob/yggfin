...
from rekep.market.identity import ABSENT, H128, SEPARATOR, arrow_of, h128_arrow, h128_of, uuids_of
...
def test_an_identifier_is_sixteen_bytes_and_nothing_else() -> None:
    built = h128_of("Order", "AAPL")
    assert isinstance(built, uuid.UUID)
    assert len(built.bytes) == 16
    assert h128_arrow("Order", pyarrow.array(["AAPL"])).type == H128 == pyarrow.binary(16)
...
def test_a_part_that_contains_the_separator_cannot_impersonate_two_parts() -> None:
    ...
    assert h128_of(smuggled) != h128_of("A", "B")
    assert h128_of("1:A:1:B") != h128_of("A", "B")


def test_a_part_holding_the_absent_marker_is_not_a_missing_one() -> None:
    assert h128_of(ABSENT.decode()) != h128_of(None)
...
def test_the_column_builder_agrees_with_the_scalar_one_row_by_row() -> None:
    ...
    column = h128_arrow("Order", pyarrow.array(SYMBOLS), pyarrow.array(IDS))
    assert uuids_of(column) == [
        h128_of("Order", symbol, identifier)
...
def test_a_sliced_batch_hashes_the_rows_it_actually_holds() -> None:
    ...
    whole = uuids_of(h128_arrow("Order", symbols, ids))
    sliced = uuids_of(h128_arrow("Order", symbols.slice(1, 3), ids.slice(1, 3)))
...
def test_a_chunked_column_hashes_as_one_column() -> None:
    ...
    assert uuids_of(h128_arrow("Order", chunked, pyarrow.array(IDS))) == uuids_of(
        h128_arrow("Order", pyarrow.array(SYMBOLS), pyarrow.array(IDS))
    )
...
def test_a_wide_string_column_hashes_the_same_as_a_narrow_one() -> None:
    ...
    assert uuids_of(h128_arrow("Order", wide)) == uuids_of(h128_arrow("Order", narrow))
...
def test_an_identifier_column_can_itself_be_a_part() -> None:
    ...
    parents = arrow_of([h128_of("a"), h128_of("b")])
    built = h128_arrow("Book", parents)
    assert built.type == H128
    assert built[0].as_py() != built[1].as_py()


def test_no_rows_is_no_rows_rather_than_an_error() -> None:
    assert len(h128_arrow("Order", pyarrow.array([], type=pyarrow.string()))) == 0
...
def test_a_column_round_trips_through_python_and_back() -> None:
    column = h128_arrow("Order", pyarrow.array(SYMBOLS))
...
def test_a_part_is_its_bytes_and_nothing_more() -> None:
    ...
    assert h128_of(b"AB") == h128_of("AB")
    assert h128_of(b"") == h128_of("")
    assert h128_of(65) == h128_of("65")
...
def test_the_column_builder_agrees_on_every_adversarial_tuple(parts: tuple[object, ...]) -> None:
    ...
    assert uuids_of(h128_arrow(*columns)) == [h128_of(*parts)]
