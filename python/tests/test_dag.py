...
def chain() -> Dag:
    ...
    for name in ("extract", "transform", "load"):
        declare(f"rekep:/jobs/pipeline/{name}")
    return Dag(
        uri="rekep:/dags/pipeline/demo",
        tasks=[
            "rekep:/jobs/pipeline/load",
            "rekep:/jobs/pipeline/extract",
            "rekep:/jobs/pipeline/transform",
        ],
...
def test_dag_id_and_namespace_read_the_levels_back_out() -> None:
    dag = Dag(uri="rekep:/dags/pipeline/trading_logs")
...
def test_uri_is_scoped_to_the_dag_scheme() -> None:
    ...
    assert str(Dag(uri="pipeline/orders").resource_uri()) == "rekep:/dags/pipeline/orders"
    assert Dag(uri="rekep:/dags/pipeline/orders").resource_uri() != (
        Job(uri="rekep:/jobs/pipeline/orders").resource_uri()
    )


def test_a_task_is_named_by_the_dag_that_runs_it() -> None:
    """OpenLineage's `dag_id.task_id`: two dags may both run a `load`."""
    job = declare("rekep:/jobs/pipeline/load")
    assert Dag(uri="rekep:/dags/pipeline/demo").task_name(job) == "demo.load"
    assert Dag(uri="rekep:/dags/pipeline/other").task_name("load") == "other.load"
...
def test_tasks_resolve_to_the_registered_objects() -> None:
    ...
    dag = Dag(uri="rekep:/dags/pipeline/demo", tasks=["rekep:/jobs/pipeline/extract"])
...
def test_two_tasks_sharing_an_id_are_refused() -> None:
    ...
    declare("rekep:/jobs/a/load")
    declare("rekep:/jobs/b/load")
    dag = Dag(uri="rekep:/dags/pipeline/demo", tasks=["rekep:/jobs/a/load", "rekep:/jobs/b/load"])
...
def test_order_falls_back_to_declaration_order() -> None:
    ...
    dag = Dag(
        uri="rekep:/dags/pipeline/demo",
        tasks=["rekep:/jobs/pipeline/c", "rekep:/jobs/pipeline/a", "rekep:/jobs/pipeline/b"],
    )
    assert dag.order() == ["c", "a", "b"]


def test_a_cycle_is_refused_by_name() -> None:
    declare("rekep:/jobs/pipeline/a")
    declare("rekep:/jobs/pipeline/b")
    dag = Dag(
        uri="rekep:/dags/pipeline/demo",
...
def test_an_edge_naming_an_undeclared_task_is_refused() -> None:
    ...
    dag = Dag(
        uri="rekep:/dags/pipeline/demo",
        tasks=["rekep:/jobs/pipeline/a"],
        dependencies={"a": ["nowhere"]},
    )
...
def test_run_walks_the_order_and_returns_what_each_task_returned() -> None:
    declare("rekep:/jobs/pipeline/first", source=SAMPLE.as_uri())
    declare("rekep:/jobs/pipeline/second", source=SAMPLE.as_uri())
    dag = Dag(
        uri="rekep:/dags/pipeline/demo",
        tasks=["rekep:/jobs/pipeline/second", "rekep:/jobs/pipeline/first"],
...
def test_a_failing_task_stops_the_run() -> None:
    ...
    declare("rekep:/jobs/pipeline/first")  # no source: extract() raises
    declare("rekep:/jobs/pipeline/second", source=SAMPLE.as_uri())
    dag = Dag(
        uri="rekep:/dags/pipeline/demo",
        tasks=["rekep:/jobs/pipeline/first", "rekep:/jobs/pipeline/second"],
        dependencies={"second": ["first"]},
    )
...
def test_a_record_two_tasks_read_is_listed_once() -> None:
    ...
    dag = Dag(
        uri="rekep:/dags/pipeline/demo", tasks=["rekep:/jobs/pipeline/a", "rekep:/jobs/pipeline/b"]
    )
    assert dag.consumed_records() == [Log]
...
def test_from_job_builds_the_one_task_dag() -> None:
    job = declare("rekep:/jobs/pipeline/extract", schedule="@daily", tags={"stage": "ingestion"})
    dag = Dag.from_job(job)
    assert str(dag.resource_uri()) == "rekep:/dags/pipeline/extract"
    assert dag.tasks == ["rekep:/jobs/pipeline/extract"]
...
def test_from_job_takes_overrides() -> None:
    job = declare("rekep:/jobs/pipeline/extract")
    assert Dag.from_job(job, uri="rekep:/dags/other/name").dag_id() == "name"
...
def test_load_needs_no_class_declared(tmp_path: pathlib.Path) -> None:
    ...
    path.write_text("uri: rekep:/dags/pipeline/demo\ntasks: [rekep:/jobs/pipeline/a]\n")
