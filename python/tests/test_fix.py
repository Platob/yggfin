...
def lines() -> pyarrow.Table:
    """The whole bridge corpus as `parse_messages` reads it: the raw contract,
    the line's own identity on every row."""
    handle = IOBase.from_uri(FIXTURE.as_uri())
    try:
        return handle.read_arrow_reader(options=Message.text_options()).read_all()
    finally:
        handle.close()


@pytest.fixture(scope="module")
def bronze() -> pyarrow.Table:
    """The whole bridge corpus, through the first stage the task runs."""
    handle = IOBase.from_uri(FIXTURE.as_uri())
    try:
        return _bronze(handle)
    finally:
        handle.close()


@pytest.fixture(scope="module")
def silver(bronze: pyarrow.Table) -> pyarrow.Table:
    """The same corpus through the second stage, over the first one's rows."""
    return _silver(bronze)
...
def test_the_stored_row_holds_none_of_the_text_it_was_read_from(bronze) -> None:
...
def test_a_message_stating_no_clock_answers_the_same_identity_on_every_read(tmp_path) -> None:
    ...
    try:
        first, second = _bronze(handle), _bronze(handle)
    finally:
        handle.close()
...
def test_the_parse_places_no_message_in_a_chain(bronze) -> None:
    """Bronze has no chain, and says so.
    ...
    """
    assert bronze.column(CHAIN_STEP).null_count == bronze.num_rows
    ...
    assert bronze.column("state").null_count == 0
    assert bronze.column("creaunix").null_count == 0
    messages = list(fix_row_messages(_codec(), _reader(bronze)))
...
def test_a_message_names_the_stored_line_it_was_parsed_out_of(lines, bronze) -> None:
    ...
    sources = bronze.column(SOURCES).to_pylist()
...
def test_every_restatement_of_an_event_settles_on_one_identity(bronze) -> None:
    ...
    for identity, source in zip(
        bronze.column(MESSAGE_KEY).to_pylist(), _sources(bronze), strict=True
    ):
...
def test_a_market_fact_is_fixs_own_field_and_the_trait_answers_off_it(bronze) -> None:
    ...
    beside them."""
    rows = bronze.select(("price", "lastpx", "avgpx", "orderqty", "lastqty")).to_pylist()
    messages = list(fix_row_messages(_codec(), _reader(bronze)))
...
def test_the_instrument_columns_say_what_the_capture_says(bronze) -> None:
    """`symbol` is what `Symbol(55)` stated, and a market that said nothing
    about trading is not a market that said closed."""
    symbols = set(bronze.column("symbol").to_pylist())
    ...
    for stated in ("securitytradingstatus", "tradsesstatus", "securitystatus"):
        assert bronze.column(stated).null_count == bronze.num_rows, stated


def test_the_bridge_bracket_fills_the_columns_it_names(bronze) -> None:
...
def test_the_batch_door_also_answers_messages(bronze) -> None:
    ...
    held = list(fix_row_messages(_codec(), _reader(bronze)))

    assert len(held) == MESSAGES
    assert [_msgtype(message) for message in held] == bronze.column("msgtype").to_pylist()
    assert [message.crosscode for message in held] == [
        code or "" for code in bronze.column("crosscode").to_pylist()
    ]
...
def test_the_walk_restates_the_events_and_adds_none(bronze, silver) -> None:
    ...
    assert not any(instant == UNDATED for instant in silver.column(EVENT_CLOCK).to_pylist())
...
def test_a_silver_message_follows_the_messages_before_it(silver) -> None:
    ...
    instants = dict(
        zip(
            silver.column(MESSAGE_KEY).to_pylist(),
            silver.column(EVENT_CLOCK).to_pylist(),
            strict=True,
        )
    )
    followed = [
        ...
        for identity, previous, parents, instant in zip(
            silver.column(MESSAGE_KEY).to_pylist(),
            silver.column("prevuuid").to_pylist(),
            silver.column("parentuuids").to_pylist(),
            silver.column(EVENT_CLOCK).to_pylist(),
            strict=True,
        )
...
def test_a_duplicate_is_not_a_successor(silver) -> None:
    ...
    assert sum(1 for identity in silver.column(MESSAGE_KEY).to_pylist()) > len(held)
...
def test_the_walk_settles_the_same_identities_however_often_it_runs(bronze) -> None:
    ...
    first, second = _silver(bronze), _silver(bronze)
