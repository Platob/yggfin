...
def _raw(message: FixMsg, field: int | str, default: str | None = None) -> str | None:
    ...
    return reading.raw if reading else default
...
def test_the_pipe_message_has_every_field() -> None:
    parsed = FixMsg.from_text(PIPE)
    assert len(parsed) == EXPECTED_FIELDS
    assert _raw(parsed, 58) == "hello world"
    assert _raw(parsed, 10) == "045"
...
def test_bytes_parse_like_text() -> None:
    assert FixMsg.from_text(PIPE.encode()).pairs == FixMsg.from_text(PIPE).pairs


def test_generic_building_dispatches_text_to_the_message_parser() -> None:
    assert FixMsg.from_(PIPE) == FixMsg.from_text(PIPE)
    assert FixMsg.from_(PIPE.encode()) == FixMsg.from_text(PIPE)
    assert FixMsg.into_redirects() is FixMsg.into_redirects()
...
def test_a_message_renders_back_and_parses_again() -> None:
    parsed = FixMsg.from_text(PIPE)
    again = FixMsg.from_text(parsed.into_text(), SOH)
    assert again.pairs == parsed.pairs


def test_repeated_tags_keep_every_value_in_order() -> None:
    parsed = FixMsg.from_text("8=FIX.4.2|146=2|55=AAA|55=BBB|10=000")
    assert _raws(parsed, 55) == ["AAA", "BBB"]
    assert _raw(parsed, 55) == "AAA"
...
def test_a_message_iterates_as_its_pairs() -> None:
    ...
    depends on.
    """
    parsed = FixMsg.from_text("8=FIX.4.2|35=8|54=1|10=045")
...
def test_group_entries_split_on_the_delimiter_tag() -> None:
    entries = FixMsg.from_text(MDS).group(268, members=[269, 270, 271])
    assert entries == [
        [("269", "0"), ("270", "1.1"), ("271", "5")],
        [("269", "1"), ("270", "1.2"), ("271", "7")],
    ]
...
def test_without_members_the_last_entry_ends_at_a_repeat_or_runs_on() -> None:
    entries = FixMsg.from_text(MDS).group(268)
...
def test_the_count_caps_the_entries() -> None:
    ...
    assert len(FixMsg.from_text(lied).group(268)) == 1
...
def test_get_and_values_reach_through_the_rendered_spellings() -> None:
    parsed = FixMsg.from_text(RENDERED)
    assert _raw(parsed, "side") == "1", "case-insensitive on the plain name"
    assert _raw(parsed, "PartyID") == "BRK", "the first group entry answers first"
    assert _raws(parsed, "PartyID") == ["BRK", "CLI", "dup"]
    assert _raws(parsed, "Side") == ["1"], "an exact key never mixes with rendered ones"
...
def test_get_returns_the_default_when_nothing_matches() -> None:
    parsed = FixMsg.from_text(RENDERED)
    assert not parsed.get("absent")
    assert _raw(parsed, "absent", "d") == "d"
    assert parsed.readings("absent") == []
...
def test_a_marked_key_is_log_noise_in_tag_mode() -> None:
    ...
    parsed = FixMsg.from_text("8=FIX.4.2|#54=1|55=TTF|", named=False)
    assert not parsed.get(54)
    assert _raw(parsed, 55) == "TTF"
...
def test_the_data_tags_are_the_ones_the_dictionary_types_data() -> None:
    ...
    data_tags = data_tags_of(FixRegistry(cache_dir=str(shipped)))
    assert data_tags | {"213", "1283"} == set(DATA_TAGS), (
        "the two XML length-paired fields remain delimiter-safe even though "
        "FIX Latest calls their payload String"
    )
