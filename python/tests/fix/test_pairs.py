"""`FixMessage.from_pairs`: what a key may be, and what a value comes out as."""
...
def test_a_tag_is_already_a_key() -> None:
    built = FixMessage.from_pairs([(54, "1"), ("44", "100.5")], TAGS)
...
def test_a_component_path_names_the_field_at_the_end_of_it() -> None:
    ...
    assert FixMessage.from_pairs([("Instrument.Symbol", "AAPL")], TAGS).pairs == [
        ("Instrument.55", "AAPL")
    ]
...
def test_an_entry_index_survives_onto_the_stored_key() -> None:
    ...
    built = FixMessage.from_pairs([("PartyID[0]", "A"), ("PartyID[1]", "B")], TAGS)
...
def test_a_group_entry_keeps_both_halves_of_where_it_sits() -> None:
    built = FixMessage.from_pairs([("NoPartyIDs[0].PartyID", "A")], TAGS)
...
def test_an_unknown_name_is_kept_exactly_as_it_arrived() -> None:
    ...
    built = FixMessage.from_pairs([("VenueOwnThing", "x"), ("Side", "1")], TAGS)
...
def test_without_a_dictionary_every_name_stays_a_name() -> None:
    ...
    built = FixMessage.from_pairs([("Side", "1")])
...
def test_a_key_that_is_nothing_at_all_drops_its_pair() -> None:
    assert FixMessage.from_pairs([("", "x"), ("   ", "y"), (True, "z")], TAGS).pairs == []
...
def test_text_is_itself() -> None:
    assert FixMessage.from_pairs([("Side", "1")], TAGS).get(54) == "1"


def test_a_boolean_is_the_y_or_n_a_fix_reader_accepts() -> None:
    """`True` is what Python prints and what no FIX parser reads."""
    built = FixMessage.from_pairs([("AggressorIndicator", True)], TAGS)
    assert built.get(1057) == "Y"
    assert FixMessage.from_pairs([("AggressorIndicator", False)], TAGS).get(1057) == "N"
...
def test_a_float_is_never_written_in_exponent_notation() -> None:
    ...
    built = FixMessage.from_pairs([("Price", 1e-07), ("OrderQty", 1e18)], TAGS)
...
def test_a_float_keeps_the_value_it_round_trips_as() -> None:
    for value in (100.5, 0.1, 1 / 3, 1e-7, 123456789.123456):
        text = FixMessage.from_pairs([("Price", value)], TAGS).get(44)
        assert float(text) == value, text


def test_an_integer_is_its_digits() -> None:
    assert FixMessage.from_pairs([("OrderQty", 10)], TAGS).get(38) == "10"


def test_an_instant_is_the_utctimestamp_the_standard_fixes() -> None:
    stamped = datetime.datetime(2026, 8, 21, 10, 30, 0, 123456)
    assert (
        FixMessage.from_pairs([("TransactTime", stamped)], TAGS).get(60)
        == "20260821-10:30:00.123456"
    )


def test_a_date_and_a_time_get_their_own_spellings() -> None:
    assert FixMessage.from_pairs([("TransactTime", datetime.date(2026, 8, 21))], TAGS).get(60) == (
        "20260821"
    )
    assert FixMessage.from_pairs([("TransactTime", datetime.time(10, 30))], TAGS).get(60) == (
        "10:30:00.000000"
    )
...
def test_a_value_that_knows_its_fix_spelling_is_asked_for_it() -> None:
    ...
    assert FixMessage.from_pairs([("Side", Side.BUY)], TAGS).get(54) == "1"


def test_a_none_value_drops_its_pair_rather_than_writing_an_empty_field() -> None:
    """`54=` is a malformed message, not an absent side."""
    assert FixMessage.from_pairs([("Side", None), ("Price", 1.0)], TAGS).pairs == [("44", "1.0")]


def test_bytes_arrive_as_the_text_they_encode() -> None:
    assert FixMessage.from_pairs([("Symbol", b"AAPL")], TAGS).get(55) == "AAPL"
...
def test_order_and_repetition_are_kept_because_they_are_the_message() -> None:
    ...
    built = FixMessage.from_pairs([("448", "A"), ("448", "B"), ("54", "1")], TAGS)
