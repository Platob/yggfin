...
def _pairs(array: pyarrow.Array, row: int = 0) -> list[tuple[object, str]]:
    """One map or list cell in the pair form the assertions read."""
    return [
...
def test_a_declared_vendor_field_is_lifted_into_a_log_column(
    ...
    pairs = codec.into_pairs(pyarrow.array([line]), "UL")
    columns, rest = codec.into_named_columns(pairs, "9.1")
    ...
    dotted = codec.into_pairs(
        pyarrow.array(["toBridge #FAKE.VENDOR.CODE=FAKE-CODE-0002|#X=1"]), "UL"
    )
    assert codec.into_named_columns(dotted, "9.1")[0]["fake_vendor_code"].to_pylist() == [
        "FAKE-CODE-0002"
    ]
...
def test_a_codec_over_a_dictionary_that_declares_none_lifts_none(store: Offline) -> None:
    ...
    pairs = codec.into_pairs(pyarrow.array(["toBridge #FAKEVENDORCODE=x|#Y=1"]), "UL")
    columns, rest = codec.into_named_columns(pairs, "9.1")
    assert columns == {}
...
def test_two_vendor_namespaces_of_one_name_stay_two_fields(store: Offline, tmp_path: Path) -> None:
    ...
    columns, rest = codec.into_named_columns(codec.into_pairs(pyarrow.array([line]), "UL"), "9.1")
