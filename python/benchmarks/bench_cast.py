...
def cases(rows: int) -> list[tuple[str, Callable[[], object], Callable[[], object] | None]]:
    ...
    struct_field = Field(name="venue", dtype=WIDER_VENUE, nullable=True)
    list_array = batch.column("legs")
    list_field = Field(
        name="legs", dtype=pyarrow.list_(pyarrow.field("item", WIDER_VENUE)), nullable=True
    )
    map_array = batch.column("tags")
    map_field = Field(
        name="tags", dtype=pyarrow.map_(pyarrow.string(), pyarrow.int32()), nullable=True
    )
    ...
    struct_of_map = Field(
        name="tags",
        dtype=pyarrow.struct([("a", pyarrow.int64()), ("b", pyarrow.int64())]),
        nullable=True,
    )
    map_of_struct = Field(
        name="venue", dtype=pyarrow.map_(pyarrow.string(), pyarrow.string()), nullable=True
    )
    list_of_struct = Field(name="venue", dtype=pyarrow.list_(pyarrow.string()), nullable=True)
    large_list = Field(name="legs", dtype=pyarrow.large_list(WIDER_VENUE), nullable=True)
    ...
    return [
        ...
        (
            ...
            lambda: list_array.cast(list_field.dtype),
        ),
        (
            "map, narrowed value",
            lambda: map_field.cast_arrow_array(map_array),
            lambda: map_array.cast(map_field.dtype),
        ),
        (
        ...
        (
            ...
            lambda: list_array.cast(large_list.dtype),
        ),
    ]
...
def verify(rows: int) -> None:
    ...
    for name in ("venue", "legs", "tags"):
        ...
        theirs = batch.column(name).cast(field.field(name).dtype)
