...
`xhash` its lifecycle, and `linked_events` holds ordered `(unix, xhash)`
...
`unix_source` says which rung answered — `TrdRegTimestamps=1`, `TransactTime`,
...
- `unix_partition` is recomputed from the resolved `unix`, and rows move
...
for purge in (False, True):
    ...
    print(purge, [(book.bid_px, book.bid_qty, book.state.name) for book in rows])
```
