...
- **`uri`** is the identity, the same path spelling everything else uses:
  `dag:/pipeline/trading_logs` and `job:/pipeline/trading_logs` are two
...
dag = find("dag:/pipeline/trading_logs")
...
Dag.from_job(job)     # dag:/<the job's own path>, one task, no edges
```
