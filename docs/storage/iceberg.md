...
The environment is the third way to state the endpoint, read the way the AWS
CLI reads it, and a default for every catalog the worker runs rather than a
setting of one: a `uri` stated outright wins, then the locator's endpoint,
then the environment, then the partition's regional endpoint.

| variable | states | when nothing above it does |
| --- | --- | --- |
| `AWS_ENDPOINT_URL_S3TABLES` | the `uri` for an ARN or a locator | `/iceberg` is added unless the value ends in it |
| `AWS_ENDPOINT_URL_GLUE` | the `uri` for a `<account>:s3tablescatalog/<name>` | the same |
| `AWS_ENDPOINT_URL_S3` | `s3.endpoint`, where the table files are read and written | |
| `AWS_ENDPOINT_URL` | `s3.endpoint` only, after `AWS_ENDPOINT_URL_S3` | |
| `AWS_IGNORE_CONFIGURED_ENDPOINT_URLS=true` | none of the above is read | |
...
for both, so it is never read for the `uri`.
