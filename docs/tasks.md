...
```text
Text files -> parse_fix -> fixmessage.market -+-> flatten_instruments -> Instrument
                            (FixMessage stream)  `-> parse_market -> Book -+-> Order
                                                                  `-> Execution
```
...
interval into each notebook. `parse_fix` starts the instrument and market
