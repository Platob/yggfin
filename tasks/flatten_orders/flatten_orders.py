...
def _(
    ...
    orders = store.dataset(
        stage.targets["orders"],
...
@app.cell
def _(books, commit_batch_num, commit_row_size, lower, merge_by, orders, upper):
    counts = {"read": 0}
...
def _(counts, stage, written):
    stage.says("projected %d orders out of the books in the window", counts["read"])
