...
Shipped artifacts are drift-tested: a dataset side file's generated schema
block fails CI when it lags the record (`rekep dataset sync --dry-run` exits
1, and the projection onto a protocol refuses outright).
...
│                  out, generated by `rekep dataset sync` and refused by
│                  verify() when it drifts (there is no `product/` folder:
│                  a data product is described in one file);
│                  read_arrow_reader/write_arrow_reader dispatch straight to
│                  the `{format}_..._arrow_reader` hook -- pyiceberg's
...
described**: `schema:`/`uri:`/`protocols:` plus the generated
`description:`/`fields:` block `rekep dataset sync` writes -- deployed
autonomously into whichever `--target` names, no `tables/` folder anywhere
and no protocol-adapted fields committed to disk). Generated DDL is
...
`sync` is one verb over every folder of declarations (`_sync_folder`):
load, materialise what the record derives, write back complete -- a templated
file is skipped, because rewriting it would resolve the template against
